...
def test_nahiku_more_synthetic():
    ...
    n2 = Nahiku.from_synthetic_parameterized_noise(
        period=5.0, amp=0.5, phase=0.1, slope=0.001
    )
...
def test_nahiku_inject_other_shapes():
    ...
    n.inject_anomaly(
        1, absolute_width=0.5, absolute_depth=5, shapes=["tophat"], idxs=[60]
    )
    # Trigger invalid shape warning
    n.inject_anomaly(
        1, absolute_width=0.5, absolute_depth=5, shapes=["invalid"], idxs=[80]
    )
