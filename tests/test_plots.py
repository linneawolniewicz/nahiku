...
def test_exhaustive_plots(tmp_path):
    ...
    es.search_for_anomaly(min_anomaly_len=2, max_anomaly_len=3, plot=True, training_iterations=1)
    es.plot()
    
    # Test save_to_txt branch
    filename = tmp_path / "results.txt"
    es.search_for_anomaly(filename=str(filename), min_anomaly_len=2, max_anomaly_len=3, training_iterations=1)
