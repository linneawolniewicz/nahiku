...
def test_nahiku_inject_anomaly():
    ...
    nahiku.inject_anomaly(1, absolute_width=0.5, absolute_depth=5, shapes=["exocomet"], idxs=[30])
    nahiku.inject_anomaly(1, absolute_width=0.5, absolute_depth=5, shapes=["gaussian"], idxs=[60])
    nahiku.inject_anomaly(1, absolute_width=0.5, absolute_depth=5, shapes=["tophat"], idxs=[80])
...
def test_nahiku_exhaustive_search():
    ...
    y[10:12] -= 10.0
    nahiku = Nahiku(x, y)
    res = nahiku.exhaustive_search(min_anomaly_len=2, max_anomaly_len=3, training_iterations=5, plot=False)
    assert res is not None
...
def test_from_lightkurve(mock_search):
    ...
    mock_search.return_value.download_all.return_value.stitch.return_value.remove_nans.return_value = mock_lc
